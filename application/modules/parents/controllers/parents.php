...
//Modules::run('site_security/has_permission');
    ...
    function submit() {
            ...
            else
            {
                ...
                $this->session->set_flashdata('message', 'parents'.' '.DATA_SAVED);
		        $this->session->set_flashdata('status', 'success');
            redirect(ADMIN_BASE_URL . 'parents');
        }
    ...
    function set_publish() {
        ...
        redirect(ADMIN_BASE_URL . 'parents/manage/' . '');
    }
    ...
    function logout_user () {
        ...
        $this->load->model('mdl_parent');
        $check = $this->mdl_parent->_logout_user($user_id,$org_id,$username);
        if($check >= 1){
            echo "1";
        }
        else{
    ...
    function _getItemById($id) {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_getItemById($id);
    }
    function _set_publish($arr_col) {
        $this->load->model('mdl_parent');
        $this->mdl_parent->_set_publish($arr_col);
    }

    function _set_unpublish($arr_col) {
        $this->load->model('mdl_parent');
        $this->mdl_parent->_set_unpublish($arr_col);
    }

    function _get($order_by) {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_get($order_by);
    }

    function _get_by_arr_id($arr_col) {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_get_by_arr_id($arr_col);
    }
    function _insert($data) {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_insert($data);
    }

    function _update($arr_col, $org_id, $data) {
        $this->load->model('mdl_parent');
        $this->mdl_parent->_update($arr_col, $org_id, $data);
    }

    function _update_id($id, $data) {
        $this->load->model('mdl_parent');
        $this->mdl_parent->_update_id($id, $data);
    }

    function _delete($arr_col, $org_id) {       
        $this->load->model('mdl_parent');
        $this->mdl_parent->_delete($arr_col, $org_id);
    }
    function _get_by_arr_id_teacher() {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_get_by_arr_id_teacher();
    }
    function _get_by_arr_id_parent() {
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_get_by_arr_id_parent();
    }

    function _get_where_validate($phone){
        $this->load->model('mdl_parent');
        $query = $this->mdl_parent->_get_where_validate($phone);
        return $query;
    }

    function _get_session($id,$username,$org_id){
        $this->load->model('mdl_parent');
        return $this->mdl_parent->_get_session($id,$username,$org_id);
    }
